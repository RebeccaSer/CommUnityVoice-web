...
const IssueForm = ({ onIssueCreated, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!currentUser?.area_id) {
      setError('You must have an area assigned to report issues.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      await api.issues.create(formDataToSend);
      onIssueCreated();
    } catch (error) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 mb-8 border border-accent2">
      <h2 className="text-2xl font-bold text-dark mb-2">Report New Community Issue</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <select
            ...
            className="w-full p-3 border border-accent2 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition-colors"
            required
          >
            {issueTypes.map(type => (
        ...
        {/* Issue Title */}
        <div className="border-l-4 border-primary pl-4 bg-primary bg-opacity-10 py-2">
        ...
        <div>
          ...
          <textarea
            ...
            rows="5"
            className="w-full p-3 border border-accent2 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition-colors resize-vertical"
            required
          />
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-4">
          <button
            type="submit"
            disabled={loading}
            className="flex-1 bg-primary hover:bg-accent1 disabled:bg-gray-300 text-white font-medium py-3 px-6 rounded-lg transition-all duration-200 flex items-center justify-center"
          >
          ...
          <button
            ...
            onClick={onCancel}
            disabled={loading}
            className="flex-1 bg-accent2 hover:bg-accent1 disabled:bg-gray-300 text-white font-medium py-3 px-6 rounded-lg transition-all duration-200"
          >
            Cancel
          </button>
        </div>
