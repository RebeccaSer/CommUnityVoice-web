...
const AreaSelector = ({ onAreaSelect }) => {
  ...
  const areaTypes = [
    { value: 'all', label: 'All Areas', color: 'primary' },
  ...
  const inactiveColorClasses = {
    primary: 'bg-primary/20 text-primary-800 hover:bg-primary/30',
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-8 border border-accent2">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-dark mb-2">Select Your Area</h2>
      ...
      <div className="mt-4 relative">
        <button
          ...
          className="w-full md:w-96 bg-gray-50 border border-accent2 rounded-lg p-3 text-left flex justify-between items-center hover:border-primary focus:outline-none focus:ring-2 focus:ring-primary"
        >
          <span className={selectedArea ? 'text-dark' : 'text-gray-500'}>
        ...
        {/* Dropdown Menu */}
        {showDropdown && (
          <div className="absolute z-10 mt-1 w-full md:w-96 bg-white border border-accent2 rounded-lg shadow-lg max-h-96 overflow-y-auto">
            <button
              onClick={() => handleAreaSelect(null)}
              className="w-full text-left px-4 py-3 hover:bg-gray-100 border-b border-accent2 text-primary font-medium"
            >
            ...
            {filteredAreas.length === 0 ? (
              ...
              filteredAreas.map(area => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 hover:bg-gray-100 border-b border-accent2 last:border-b-0
                    ${selectedArea?.id === area.id ? 'bg-primary/10 text-primary' : 'text-gray-700'}`}
                >
      ...
      {/* Quick Tips */}
      <div className="mt-4 text-sm text-gray-500 bg-primary/10 p-3 rounded-lg border border-primary/20">
